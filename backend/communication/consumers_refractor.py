...
class Rider(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        self.rider_details=dict(serializer.data) #{name: ,email: ,phone_number}
    ...
    async def receive(self,text_data=None):
        ...
        if frontend_message.get("review",0):
            frontend_message.pop("review")
            await models.RideDetails.objects.filter(id=self.rider_id).aupdate(**frontend_message)
            logger.info(f"[RIDER] Reviewed : {frontend_message}")
        else:
    ...
    async def request_accepted(self,event):
        ...
        logger.success(f"[RIDER] Got response from DRIVER → {message}")
        rider = await models.Rider.objects.aget(id=self.rider_id)
        driver = await models.Driver.objects.aget(id=message["id"])
        created = await models.RideDetails.objects.acreate(
            rider=rider,
            driver=driver,
            source="",
            destination="",
